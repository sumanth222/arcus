...
//        lib(repo, "Smith Machine Bench Press", "chest",      "triceps,shoulders", "middle",   "smith_machine","compound",  "Bar path is fixed — focus on a strong leg drive.");
        ...
        // ── BACK ──────────────────────────────────────────────────────────────────
//        lib(repo, "Deadlift",                  "back",       "legs,legs", "lower",    "barbell",      "compound",  "Neutral spine from start to finish; push the floor away.");
//        lib(repo, "Barbell Row",               "back",       "biceps",            "mid",      "barbell",      "compound",  "Hinge to ~45°; pull the bar to the lower belly.");
//        lib(repo, "Dumbbell Row",              "back",       "biceps",            "mid",      "dumbbell",     "compound",  "Brace the non-working arm; pull elbow past your hip.");
//        lib(repo, "Pull-Up",                   "back",       "biceps",            "lats",     "bodyweight",   "compound",  "Full dead hang at the bottom; chin over the bar at the top.");
//        lib(repo, "Lat Pulldown",              "back",       "biceps",            "lats",     "machine",      "compound",  "Lean back slightly; pull the bar to your upper chest.");
//        lib(repo, "Seated Cable Row",          "back",       "biceps",            "mid",      "cable",        "compound",  "Keep chest tall; squeeze shoulder blades together at the end.");
//        lib(repo, "T-Bar Row",                 "back",       "biceps",            "mid",      "barbell",      "compound",  "Use a neutral grip and a slight knee bend throughout.");
//        lib(repo, "Face Pull",                 "back",       "shoulders",         "upper",    "cable",        "isolation", "Pull to forehead level and externally rotate at the end.");
//        lib(repo, "Machine Row",               "back",       "biceps",            "mid",      "machine",      "compound",  "Adjust chest pad so arms are parallel to the floor.");

        // ── SHOULDERS ─────────────────────────────────────────────────────────────
//        lib(repo, "Barbell Overhead Press",    "shoulders",  "triceps",           "front",    "barbell",      "compound",  "Press straight up; don't overarch the lower back.");
//        lib(repo, "Dumbbell Shoulder Press",   "shoulders",  "triceps",           "front",    "dumbbell",     "compound",  "Lower dumbbells to ear height for a full range of motion.");
//        lib(repo, "Lateral Raise",             "shoulders",  "",                  "side",     "dumbbell",     "isolation", "Lead with your elbows, not your hands; stop at shoulder height.");
//        lib(repo, "Cable Lateral Raise",       "shoulders",  "",                  "side",     "cable",        "isolation", "Cable provides constant tension — great for the mid-delt.");
//        lib(repo, "Machine Shoulder Press",    "shoulders",  "triceps",           "front",    "machine",      "compound",  "Adjust seat so handles align with your shoulders.");
//        lib(repo, "Front Raise",               "shoulders",  "",                  "front",    "dumbbell",     "isolation", "Keep a slight bend in the elbows; don't swing.");
//        lib(repo, "Rear Delt Fly",             "shoulders",  "back",              "rear",     "dumbbell",     "isolation", "Hinge forward; lead with your elbows wide.");
//        lib(repo, "Cable Face Pull",           "shoulders",  "back",              "rear",     "cable",        "isolation", "Use a rope attachment; pull to eye level and rotate out.");

        // ── legs ─────────────────────────────────────────────────────────────────
//        lib(repo, "Barbell Squat",             "legs",      "legs,legs", "overall",  "barbell",      "compound",  "Break at hips and knees simultaneously; keep chest tall.");
//        lib(repo, "Leg Press",                 "legs",      "legs,legs", "overall",  "machine",      "compound",  "Place feet shoulder-width; don't lock out knees at the top.");
//        lib(repo, "Hack Squat",                "legs",      "legs",        "overall",  "machine",      "compound",  "Keep heels on the platform; go as deep as mobility allows.");
//        lib(repo, "Leg Extension",             "legs",      "",                  "overall",  "machine",      "isolation", "Pause briefly at the top; don't use momentum.");
//        lib(repo, "Bulgarian Split Squat",     "legs",      "legs,legs", "overall",  "dumbbell",     "compound",  "Rear foot elevated; most weight through the front heel.");
//        lib(repo, "Smith Machine Squat",       "legs",      "legs,legs", "overall",  "smith_machine","compound",  "Position feet slightly forward for a more vertical shin angle.");
//
//        // ── legs ────────────────────────────────────────────────────────────
//        lib(repo, "Romanian Deadlift",         "legs", "legs",            "overall",  "barbell",      "compound",  "Soft knees; push hips back until you feel a hamstring stretch.");
//        lib(repo, "Leg Curl",                  "legs", "",                  "overall",  "machine",      "isolation", "Flex at the top; lower slowly for a 3-second negative.");
//        lib(repo, "Stiff-Leg Deadlift",        "legs", "legs",            "overall",  "dumbbell",     "compound",  "Keep the dumbbells close to your legs the whole way down.");

        // ── legs ────────────────────────────────────────────────────────────────
//        lib(repo, "Hip Thrust",                "legs",     "legs",        "overall",  "barbell",      "compound",  "Drive through your heels; fully extend the hip at the top.");
//        lib(repo, "Glute Kickback",            "legs",     "legs",        "overall",  "cable",        "isolation", "Keep the core braced; isolate the glute — don't hyperextend the back.");
...
//        lib(repo, "Hammer Curl",               "biceps",     "forearms",          "long_head", "dumbbell",     "isolation", "Neutral grip targets the brachialis for arm thickness.");
...
//        lib(repo, "EZ Bar Curl",               "biceps",     "",                  "short_head","ez_bar",       "isolation", "Angled grip reduces wrist strain; keep elbows tucked.");
        ...
        // ── TRICEPS ───────────────────────────────────────────────────────────────
        lib(repo, "Tricep Pushdown",           "triceps",    "",                  "lateral_head","cable",      "isolation", "Keep elbows fixed at your sides; fully extend and squeeze.");
        ...
        lib(repo, "Close-Grip Bench Press",    "triceps",    "chest",             "medial_head","barbell",     "compound",  "Hands just inside shoulder-width; tuck elbows at 45°.");

        // ── CORE ──────────────────────────────────────────────────────────────────
//        lib(repo, "Plank",                     "core",       "",                  "overall",  "bodyweight",   "isolation", "Brace abs as if bracing for a punch; breathe steadily.");
//        lib(repo, "Cable Crunch",              "core",       "",                  "upper",    "cable",        "isolation", "Round the spine; pull down with your abs, not your hip flexors.");
//        lib(repo, "Hanging Leg Raise",         "core",       "",                  "lower",    "bodyweight",   "isolation", "Control the swing; exhale as you raise the legs.");
//        lib(repo, "Ab Wheel Rollout",          "core",       "",                  "overall",  "bodyweight",   "compound",  "Brace hard before rolling out; don't let your lower back sag.");
    }

    /**
     * Creates and saves one ExerciseLibraryEntity using setters.
     * Uses the no-arg constructor so this method stays compatible with any future
     * field additions to ExerciseLibraryEntity.
     *
     * Defaults applied:
     *   level           = "beginner,intermediate,advanced"
     *   workoutLocation = "both" for bodyweight exercises, "gym" for everything else
     *   sets / repMin / repMax — sensible defaults based on category
     */
    private void lib(ExerciseLibraryRepo repo,
        ...
        if (repo.existsByNameIgnoreCase(name)) return;

        ExerciseLibraryEntity e = new ExerciseLibraryEntity();
        e.setName(name);
        e.setMuscleGroup(muscleGroup);
        e.setSecondaryMuscles(secondaryMuscles);
        e.setMuscleArea(muscleArea);
        e.setEquipment(equipment);
        e.setCategory(category);
        e.setTip(tip);
        e.setLevel("beginner,intermediate,advanced");
        e.setWorkoutLocation("bodyweight".equals(equipment) ? "both" : "gym");

        // Sensible rep/set defaults — update per-exercise in the DB for finer control
        boolean isCompound = "compound".equals(category);
        e.setSets(3);
        e.setRepMin(isCompound ? 5 : 10);
        e.setRepMax(isCompound ? 10 : 15);
